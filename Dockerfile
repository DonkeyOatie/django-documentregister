...
RUN mkdir -p /var/log/documentregister

ADD conf/entrypoint.sh /usr/local/bin/entrypoint.sh 
RUN chmod +x /usr/local/bin/entrypoint.sh

RUN mkdir -p /etc/supervisor/conf.d && mkdir -p /var/log/supervisor
RUN ln -sf /code/conf/supervisor.documentregister.conf /etc/supervisor/conf.d/documentregister.conf
RUN ln -sf /code/conf/supervisord.conf /etc/supervisor/supervisord.conf

ADD conf/nginx.documentregister.conf /etc/nginx/sites-enabled/default

RUN mkdir -p /var/www/static && chmod -R 760 /var/www/static/ && chown -R www-data:www-data /var/www/static
RUN mkdir -p /opt/documentregister/attachments && chmod -R 700 /opt/documentregister/attachments && chown -R www-data:www-data /opt/documentregister/attachments
